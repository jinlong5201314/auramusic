...
// ─── 国内节点穿透请求头（穿透酷狗、酷我海外 IP 拦截）────────────────────────
const CHINA_FORWARD_HEADERS: Record<string, string> = {
...
interface UnifiedSong {
  ...
  source_name: string;
  duration: number; // 秒
  formats: string[]; // ["128k", "320k", "flac", "flac24bit"]
  score?: number;
}
...
function zzcSign(text: string): string {
  ...
  let binary = "";
  for (let i = 0; i < part3Bytes.length; i++) {
    binary += String.fromCharCode(part3Bytes[i]);
  }
  const b64 = btoa(binary).replace(/[\/+=]/g, "");
  return `zzc${part1}${b64}${part2}`.toLowerCase();
}

async function searchTx(keyword: string, page = 1, limit = 20): Promise<UnifiedSong[]> {
  try {
    ...
    return songs.map((s: any) => {
      ...
      const singer = (s.singer || []).map((sg: any) => sg.name).join(" / ");
      const mid = s.mid || "";
...
// ─── 小芸音乐 (网易云/wy) ─────────────────────────────────────────────────────
async function searchWy(keyword: string, page = 1, limit = 20): Promise<UnifiedSong[]> {
  try {
    ...
    if (resp.ok) {
      ...
      if (Array.isArray(songs) && songs.length > 0) {
        return songs.map((s: any) => {
          const artists = (s.artists || []).map((a: any) => a.name).join(" / ");
...
// ─── 小蜗音乐 (酷我/kw) ───────────────────────────────────────────────────────
async function searchKw(keyword: string, page = 1, limit = 20): Promise<UnifiedSong[]> {
  try {
    const pn = page - 1;
...
// ─── 小枸音乐 (酷狗/kg) ───────────────────────────────────────────────────────
async function searchKg(keyword: string, page = 1, limit = 20): Promise<UnifiedSong[]> {
  try {
...
// ─── 小蜜音乐 (咪咕/mg) ───────────────────────────────────────────────────────
async function searchMg(keyword: string, page = 1, limit = 20): Promise<UnifiedSong[]> {
  try {
    ...
    const res: UnifiedSong[] = [];
    for (const group of resultList) {
      const items = Array.isArray(group) ? group : [group];
      for (const s of items) {
        if (!s || !s.name) continue;
        const singers = (s.singers || []).map((sg: any) => sg.name).filter(Boolean).join(" / ");
        const formats: string[] = [];
...
// ─── 聚合大会 (all)：并发聚合、智能去重与相似度打分 ────────────────────────────
async function searchAll(keyword: string, page = 1, limit = 20): Promise<UnifiedSong[]> {
...
// ─── Cloudflare Pages Functions 路由入口 ──────────────────────────────────
export async function onRequestGet(context: any) {
  const { request } = context;
  const url = new URL(request.url);
  const keyword = url.searchParams.get("keyword") || url.searchParams.get("name") || "";
  const source = (url.searchParams.get("source") || "all").toLowerCase();
  const page = parseInt(url.searchParams.get("page") || url.searchParams.get("pages") || "1", 10) || 1;
  const limit = parseInt(url.searchParams.get("limit") || url.searchParams.get("count") || "20", 10) || 20;

  if (!keyword.trim()) {
    return new Response(JSON.stringify({ success: false, error: "缺少搜索关键词 keyword" }), {
      status: 400,
      headers: { "Content-Type": "application/json; charset=utf-8" },
    });
  }

  let list: UnifiedSong[] = [];
  switch (source) {
    case "tx":
    case "qq":
