...
> **归档日期**：2026年9月26日  
> **当前版本**：v4.3.5 (完全移植 LXMusic Web 核心频谱律动引擎 Wave.js 与 CORS 自动代理管道版)  
...
### 7. 自定义音源更新/禁用与首页全端版本呈现 (v4.3.0)
- **单源/全量一键更新**：支持单个音源按钮或列表顶栏「🔄 全部更新」一键重新从远程 URL 拉取最新脚本，自动对比并更新版本号/作者/元数据，刷新脚本内存缓存，若为当前活跃源自动重新编译载入沙箱并同步至 D1；
- **音源禁用/启用机制**：支持单个音源状态切换，禁用后卡片置灰并标记「已禁用」，禁用源严禁参与播放调度与容灾回退；若当前激活源被禁用，系统自动优雅顺延激活下一个可用源；
- **首页及顶栏全端版本号显式展示**：桌面端在 AuraMusic 品牌标题旁展示沉浸式 `v4.3.0` 胶囊徽章（支持紧凑模式与深色自适应），移动端在顶部标题栏同步常显 `v4.3.0` 徽章，并在常量与全站资源引入中统一规范化。
